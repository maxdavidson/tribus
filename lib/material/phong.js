...
export default class PhongMaterial extends Material {
    ...
    static fromMtlFile(filename: string): Promise {
        return getString(filename)
            .then(text => {
                ...
                for (let line of text.split(/\r?\n/)) {
                    ...
                    switch (type) {
                    case 'newmtl': rawMaterials[++i] = { name: data[0] }; break;
                    //case 'Ka': rawMaterials[i].ambient = data.map(parseFloat); break;
                    case 'Kd': rawMaterials[i].diffuse = data.map(parseFloat); break;
                    //case 'Ks': rawMaterials[i].specular = data.map(parseFloat); break;
                    //case 'Ns': rawMaterials[i].shininess = parseFloat(data[0]); break;
                    case 'map_Kd':
                        promises.push((n => Texture2D.fromFile(prefix + data[0].replace(/^\//,'')).then(texture => rawMaterials[n].diffuse = texture))(i));
                    }
                }
